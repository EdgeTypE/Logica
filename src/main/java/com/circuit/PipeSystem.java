...
public class PipeSystem extends EntityTickingSystem<EntityStore> {
        ...
        if (tickCounter % 60 == 0 && !pipes.isEmpty()) {
            ...
            for (Map.Entry<Vector3i, PipeComponent> entry : pipes.entrySet()) {
                ...
                try {
                    ...
                    String itemInfo = (item == null || item.isEmpty()) ? "empty"
                            : item.getItemId() + " x" + item.getQuantity();
                    // LOGGER.atInfo().log(PREFIX + " Pipe at " + pos + ": " + itemInfo + ",
                    // connections=" + pipe.getConnectionCount());
                } catch (Exception e) {
    ...
    private void processPipe(Vector3i position, PipeComponent pipe, float deltaTime) {
        ...
        // MANTIK DÜZELTMESİ:
        // Eğer pipe boşsa item çekmeyi dene
        if (!pipe.hasItem()) {
            // tryPullItem void dönüyor, başarılı olup olmadığını anlamak için
            // pipe.hasItem() kontrolü veya cooldown kontrolü yapabiliriz.
            // Çünkü tryPullItem başarılıysa cooldown başlatıyor.

            tryPullItem(position, pipe);

            // Eğer çekme başarılı olduysa (cooldown başladıysa veya item geldiyse),
            // BU TICK İÇİNDE İTME YAPMA.
            if (pipe.isOnCooldown() || pipe.hasItem()) {
                return; // ÖNEMLİ: Zincirleme reaksiyonu kırmak için buradan çıkıyoruz.
            }
        }

        // Sadece item daha önceden varsa (bu tick çekilmediyse) itmeyi dene
        if (pipe.hasItem()) {
    ...
    private boolean hasInventoryAt(World world, int x, int y, int z) {
        try {
            ...
            com.hypixel.hytale.server.core.universe.world.meta.BlockState blockState = chunkAccessor.getState(x, y, z,
                    true);
    ...
    private boolean isInventoryBlock(String blockId) {
        ...
        if (blockId.contains("Furnace"))
            return true;
        if (blockId.contains("barrel"))
            return true;
        if (blockId.contains("Barrel"))
            return true;
        if (blockId.contains("dispenser"))
            return true;
        if (blockId.contains("Dispenser"))
            return true;
        if (blockId.contains("dropper"))
            return true;
        if (blockId.contains("Dropper"))
            return true;
            ...
            for (short slot = 0; slot < sourceInventory.getCapacity(); slot++) {
                ...
                if (removeResult.succeeded()) {
                    ...
                    if (addResult.succeeded()) {
                        ...
                        // LOGGER.atInfo().log(PREFIX + "Pulled item " + singleItem.getItemId() + " from
                        // " + sourcePos + " (" + dir + ") to pipe at " + position);
    ...
    private void tryPushItem(Vector3i position, PipeComponent pipe) {
        ...
        try {
            ...
            LOGGER.atWarning()
                    .log(PREFIX + "Error getting item from pipe buffer at " + position + ": " + e.getMessage());
            return;
        }
        ...
        if (tickCounter % 60 == 0) { // Log every 3 seconds to avoid spam
            // LOGGER.atInfo().log(PREFIX + "Pipe at " + position + " attempting to push to
        ...
        if (!isDirectionConnected(pipe, outputDir)) {
            ...
            if (tickCounter % 120 == 0) { // Log every 6 seconds to avoid spam
                // LOGGER.atInfo().log(PREFIX + "Pipe at " + position + " output direction " +
                // outputDir + " is not connected");
            }
            return;
        }
        ...
        if (targetInventory == null) {
            if (tickCounter % 120 == 0) { // Log every 6 seconds to avoid spam
                // LOGGER.atInfo().log(PREFIX + "No inventory found at output position " +
                // targetPos + " for pipe at " + position);
            }
            return;
        }
        ...
        // CRITICAL: For pipe-to-pipe transfers, prefer pipes that are empty or have
        // space
        ...
        if (targetPipe != null) {
            try {
                ...
                if (targetItem != null && !targetItem.isEmpty()) {
                    if (tickCounter % 120 == 0) { // Log every 6 seconds to avoid spam
                        // LOGGER.atInfo().log(PREFIX + "Target pipe at " + targetPos + " is full,
                        // cannot push from " + position);
                    }
                    return;
                }
        ...
        if (addResult.succeeded()) {
            ...
            if (removeResult.succeeded()) {
                ...
                // LOGGER.atInfo().log(PREFIX + "Pushed item " + item.getItemId() + " from pipe
                // at " + position + " to " + targetPos + " (" + outputDir + ")");
                return; // Successfully pushed item
            } else {
                LOGGER.atWarning().log(
                        PREFIX + "Failed to remove item from pipe buffer at " + position + " after successful push");
            ...
            if (tickCounter % 120 == 0) { // Log every 6 seconds to avoid spam
                // LOGGER.atInfo().log(PREFIX + "Could not push item " + item.getItemId() + "
                // from pipe at " + position + " to " + targetPos + " - target full or
                // invalid");
            }
        }
    }
    ...
    private boolean isDirectionConnected(PipeComponent pipe, PipeComponent.Direction direction) {
        switch (direction) {
            case NORTH:
                return pipe.isConnectedNorth();
            case SOUTH:
                return pipe.isConnectedSouth();
            case EAST:
                return pipe.isConnectedEast();
            case WEST:
                return pipe.isConnectedWest();
            case UP:
                return pipe.isConnectedUp();
            case DOWN:
                return pipe.isConnectedDown();
            default:
                return false;
        }
    }
    ...
    private List<PipeComponent.Direction> getConnectedDirections(PipeComponent pipe) {
        ...
        if (pipe.isConnectedNorth())
            directions.add(PipeComponent.Direction.NORTH);
        if (pipe.isConnectedSouth())
            directions.add(PipeComponent.Direction.SOUTH);
        if (pipe.isConnectedEast())
            directions.add(PipeComponent.Direction.EAST);
        if (pipe.isConnectedWest())
            directions.add(PipeComponent.Direction.WEST);
        if (pipe.isConnectedUp())
            directions.add(PipeComponent.Direction.UP);
        if (pipe.isConnectedDown())
            directions.add(PipeComponent.Direction.DOWN);
    ...
    private ItemContainer getInventoryAt(World world, Vector3i position) {
        try {
            ...
            if (blockState != null
                    && blockState instanceof com.hypixel.hytale.server.core.universe.world.meta.state.ItemContainerBlockState) {
                com.hypixel.hytale.server.core.universe.world.meta.state.ItemContainerBlockState containerState = (com.hypixel.hytale.server.core.universe.world.meta.state.ItemContainerBlockState) blockState;
    ...
    public void savePipes() {
        try {
            ...
            // LOGGER.atInfo().log(PREFIX + "Saved " + pipes.size() + " pipes to " +
            // SAVE_FILE);
        ...
        try {
            ...
            for (String pipeObj : pipeObjects) {
                try {
                    ...
                    // LOGGER.atInfo().log(PREFIX + "Loaded pipe at " + pos + " with output
            ...
            // LOGGER.atInfo().log(PREFIX + "Loaded " + loadedCount + " pipes from save
            // file");
