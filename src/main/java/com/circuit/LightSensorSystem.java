...
/**
 * Light Sensor System - Detects sunlight levels and emits signals based on day/night cycle.
 * 
 * Design:
 ...
 * - If sunlight factor is above threshold (default: 0.5), emits power signal (15)
...
public class LightSensorSystem extends EntityTickingSystem<EntityStore> {
    ...
    /**
     * Get the combined light level at a position (legacy method, kept for reference).
     * Combines sky light and block light, returning the maximum.
     */
    private int getLightLevelAt(World world, Vector3i pos) {
        try {
            // Calculate chunk coordinates (right shift by 5 is equivalent to dividing by 32)
            ...
            LOGGER.atWarning().log(PREFIX + "[LightSensor] Failed to get light level at " + pos + ": " + e.getMessage());
            return 0;
        }
    }
    ...
    private void updateSensorVisual(Vector3i pos, boolean powered) {
        try {
            ...
            try {
                ...
                LOGGER.atInfo().log(PREFIX + "[LightSensor] setBlockInteractionState at " + pos + " -> " + targetState);
                ...
                if (newBlockType != null) {
                    ...
                    LOGGER.atInfo().log(PREFIX + "[LightSensor] Final state set at " + pos + " -> " + targetState);
                }
                ...
                try {
                    String[] stateNames = {targetState, "State_Definitions_" + targetState, targetState.toLowerCase()};
                    
                    boolean success = false;
                    for (String stateName : stateNames) {
                        try {
                            chunkAccessor.setBlockInteractionState(pos, sensorType, stateName);
                            LOGGER.atInfo().log(PREFIX + "[LightSensor] Fallback state set at " + pos + " -> " + stateName);
                            success = true;
                            break;
                        } catch (Exception e2) {
                            // Try next state name
    ...
    public void registerSensor(Vector3i position, double threshold) {
        if (!sensorThresholds.containsKey(position)) {
            ...
            LOGGER.atInfo().log(PREFIX + "[LightSensor] Registered at " + position + " with threshold " + threshold);
        }
    }
    ...
    public void setSensorThreshold(Vector3i position, double threshold) {
        if (sensorThresholds.containsKey(position)) {
            ...
            LOGGER.atInfo().log(PREFIX + "[LightSensor] Threshold at " + position + " set to " + threshold);
        }
    }
    ...
    public void saveSensors(Path dataDirectory) {
        ...
        try {
            ...
            LOGGER.atInfo().log(PREFIX + "[LightSensor] Saved " + sensorThresholds.size() + " sensors to " + filePath);
    ...
    public void loadSensors(Path dataDirectory) {
        ...
        try (BufferedReader reader = Files.newBufferedReader(filePath)) {
            ...
            LOGGER.atInfo().log(PREFIX + "[LightSensor] Loaded " + count + " sensors from " + filePath);
    ...
    private void powerAdjacentBlocks(Vector3i sensorPos, boolean powered) {
        ...
        for (int[] offset : offsets) {
            ...
            if (plugin.getLampSystem() != null && plugin.getLampSystem().isLampAt(adjacentPos)) {
                ...
                LOGGER.atInfo().log(PREFIX + "[LightSensor] Powered lamp at " + adjacentPos + " with power=" + powerLevel);
            }
            ...
            if (plugin.getPistonSystem() != null && plugin.getPistonSystem().isPistonAt(adjacentPos)) {
                ...
                LOGGER.atInfo().log(PREFIX + "[LightSensor] Triggered piston update at " + adjacentPos + " with power=" + powerLevel);
            }
            ...
            if (plugin.getDoorSystem() != null && plugin.getDoorSystem().isDoorTracked(adjacentPos)) {
                ...
                LOGGER.atInfo().log(PREFIX + "[LightSensor] Powered door at " + adjacentPos + " with power=" + powerLevel);
            }
            ...
            if (plugin.getGateSystem() != null && plugin.getGateSystem().isGateAt(adjacentPos)) {
                ...
                LOGGER.atInfo().log(PREFIX + "[LightSensor] Triggered gate update at " + adjacentPos + " with power=" + powerLevel);
            }
            ...
            if (plugin.getRepeaterSystem() != null && plugin.getRepeaterSystem().isRepeaterAt(adjacentPos)) {
                ...
                LOGGER.atInfo().log(PREFIX + "[LightSensor] Triggered repeater update at " + adjacentPos + " with power=" + powerLevel);
            }
     ...
     * This ensures that lamps, pistons, and other circuit blocks get properly updated
